...
把一段 MRD / 原始 PRD 转化为可指导研发直接开发的 PRD-SPEC：逐项检查 → 自动补全 → 自检 → 一次性交互收集缺口 → 按模板渲染。特色：必备/条件必备/可选三级必要性判定、`≈推断` / `⚠️待确认` 风险标注、ai-kit 复用清单与规范引用（R 板块）恒定激活。

**触发方式**：提供 MRD/PRD/需求片段时自动触发；可显式要求“用 prd-spec-enhancer 完善这份需求”。

### `grilling/` — 无情拷问

把方案/计划/面试叙事映射成决策树（design tree），按轮次逐层追问，直到没有静默假设。每轮把整个 frontier（当前可问的问题集）一次问完，每个问题附推荐答案；事实查找由 AI 负责，用户只答决策类问题。
